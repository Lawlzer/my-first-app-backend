...
function saveInterface(node: ts.InterfaceDeclaration | ts.TypeAliasDeclaration, foundInterfaces: CustomInterface[]) {
	...
	const sourcePath = currentPath.replace('/dist/', '/src/').replace('.d.ts', '.ts');
	const relativesourcePath = path.relative(pathToRoutes, sourcePath);
...
function handleOneFile(inputPath: string, outputPath: string): void {
	...
	for (const interfaceSingle of foundInterfaces) {
		output += `\n// Path to source: file://${interfaceSingle.sourcePath}`;
		output += `\n// DEBUG: currentPath: file://${interfaceSingle.currentPath}`;
		output += `\n// DEBUG: originalName: ${interfaceSingle.originalName}`;
		output += `\n// DEBUG: newName: ${interfaceSingle.newName}`;
		output += `\nexport { ${interfaceSingle.originalName} } from '${interfaceSingle.sourcePath.replace('.ts', '')}';`;
		output += `\n${interfaceSingle.text.replace('export', '')}`;
		output += `\n\n\n`;
	}
